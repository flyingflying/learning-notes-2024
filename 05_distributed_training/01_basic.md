...
stage1 已经做到了模型参数的零冗余更新, 但是还有优化空间。stage2 策略流程如下:
...
和 stage1 相比, 在 stage2 中, 反向传播 和 reduce-scatter 是异步执行的关系, 同时不需要的显存也得到了及时的释放。理论上说, 通信量没有增加, 并行效率 提高了很多。
...
ZeRO 的核心思想是每个 GPU 设备维护一部分的模型参数。我们一般的说法是, stage1 拆分了 优化器状态, stage2 进一步拆分了 梯度, 而 stage3 更进一步拆分了 模型参数。从 stage1 到 stage3, 拆分的内容越来越多, 显存消耗越来越少, 是一个递进的关系。
...
在上图中, 我们假设有两个 GPU, 分别是 GPU 0 和 GPU 1。假设一开始, 输入张量 $X$ 在两个 GPU 设备上都存在。那么, 我们可以将 MLP 的第一个线性层用 **列并行** 的方式放在两个 GPU 设备上; 将 MLP 的第二个线性层用 **行并行** 的方式放在两个 GPU 设备上。
...
其中, Attention 层有一种非常直接的拆分方式, 那就是按照 "head" 进行切分。如果有两个 GPU, 那么我们就将 "head" 分成两组, 一组放在 GPU 0 上, 一组放在 GPU 1 上, 计算完成后 concat 到一起就可以了。

有没有发现, 这种切分方式和 **列并行线性层** 的切分方式是相似的。那么, 我们可以用 5.2 的策略, 对 MLP 层进行切分, 图示如下:

![图四: Attention 层切分方式](./assets/04_attn_tp.png)

在上图中, 输入矩阵 $X$ 和 输出矩阵 $Z$ 都是存在于两个 GPU 设备上的。

$f$ 和 $g$ 表示通信操作, 前向传播 时, $f$ 是 identity 操作 (输入 和 输出 相同), $g$ 是 all-reduce 操作; 反向传播 时, $f$ 是 all-reduce 操作, $g$ 是 identity 操作。因此, 整个过程也是只用到了一次 all-reduce 通信操作。
...
在上图中, $X_1^s$ 表示张量按 seq_len 维度进行切分, $X_1$ 表示 **行并行线性层** 的计算结果。如果训练时最大的序列长度是 512, 那么一个 GPU 上只维护 256 序列长度。
...
和之前的一样, `LayerNorm` 层还是由两个 GPU 共同维护。不过现在不用保证 Dropout 层 mask 的位置是一致的了。理论上说, 此时一个 GPU 只要维护一半的 **激活值**, 显存消耗也大幅度下降。
